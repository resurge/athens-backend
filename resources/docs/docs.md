...
The `home-routes` handler in the `athens-sync.routes.home` namespace
...
The `home-page` function will in turn call the `athens-sync.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `athens-sync.handler` namespace:
...
Request middleware functions are located under the `athens-sync.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `athens-sync.dev-middleware` namespace found in
